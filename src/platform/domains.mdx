...
You can check the validation status of the certificate with `kraft cloud cert get` (more on this command below).
...
* For a subdomain you own such as docs.mydomain.com, a CNAME is enough.
  In this case, you can just add a `CNAME` record with your sub-domain (`docs`) as the host and point the record to Unikraft Cloud's metro (for example, `fra0.kraft.host`).
...
* For an apex domain (for example, mydomain.com), add one of: ALIAS, ANAME, or a flattened CNAME record.
  ...
  Point the record to the metro (for example, `fra0.kraft.host`).
...
See the [certificates API](/docs/api/v1/certificates)) for details.
:::
...
Log into Unikraft Cloud by setting your token and a [metro](/metros#available) close to you.
...
With this in place, use the `kraft cloud deploy` command to create an instance of the web server and to link it to a custom name.
Unikraft Cloud does this through the `-d` flag:
...
When you issue the `-d` flag, Unikraft Cloud requests a
...
Check the validation status with `kraft cloud cert get` (more on this command below).
