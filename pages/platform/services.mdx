...
You can set both fields at service group creation time and update them later with `PATCH /v1/services/{name}`.
...
- A service group created with `kraft cloud service create` (or `POST /v1/services`) is persistent.
  It survives independently of any instances attached to it.
- A service group created implicitly via `kraft cloud deploy -p …` (shorthand port forwarding) belongs to the instance.
  The platform deletes it when you delete that instance.

The `GET /v1/services/{name}` response includes a `persistent` field (`bool`) that indicates which case applies.
