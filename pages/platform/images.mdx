...
Start with the simplest workflow: create an image from a Python app (following the Python app [guide](/guides/python)) and start an instance from it with a single `kraft cloud deploy` command:
...
Returns all images in your account.
Each image object includes:

| Field | Type | Description |
|-------|------|-------------|
| `uuid` | string | Image UUID. |
| `name` | string | Image name (if present). |
...
```json title="GET /v1/images/list"
{
  "digest": "sha256:278cb8b14f9faf9c2702dddd8bfb6124912d82c11b4a2c6590b6e32fc4049472"
}
```

The response includes `digest`, `tags`, `initrd`, `size_in_bytes`, and `args`.

:::note
Tag-based filtering (`tag` field in the list body) appears in the API schema but the platform doesn't act on it yet.
The platform accepts and ignores the field.
Use the digest or UUID endpoints to look up a specific image.
:::
