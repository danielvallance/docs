...
You can create a rootfs with the [`kraft` CLI](/cli/) for both types, for example:
...
```bash title="Attaching Volume to Instance"
kraft cloud deploy \
    -M 512 \
    -p 443:8080 \
...
kraft pkg \
    --plat kraftcloud \
    --arch x86_64 \
    --name index.unikraft.io/function-hello:latest \
    --rootfs-type erofs \
    --push \
    .
```
