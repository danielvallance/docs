...
{/* vale off */}
:::caution
Autoscale is currently only partially available via the legacy CLI (`kraft`).
Full `unikraft` CLI support is coming soon.
...
This single deploy or run flow does 4 things:

1. Builds the NGINX image and pushes it to the registry.
1. Creates an instance of NGINX which will serve as the basis for the autoscale **template**.
...
With the `unikraft` CLI, prepare a template and a service manually before enabling autoscale.
If you use the `kraft` CLI, skip ahead to the `scale init` command in the next step, which handles both for you.

First, convert the instance you deployed into a template that autoscale clones during scale-out:
...
Set them with `--warmup-time` and `--cooldown-time` (`kraft`) or `warmup_time_ms` and `cooldown_time_ms` (API).
...
```ansi title="kraft"
   ...
   warmup (ms): 1000
 cooldown (ms): 1000
      template: 8a8bc1b9-0af6-420e-a426-190dc2da9eaa
