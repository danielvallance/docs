...
:::tip
If you add instances to a [service](/platform/services), the service will load balance traffic across all them and Unikraft Cloud will ensure each instance gets woken up as needed.
...
If the count is `0`, scale-to-zero remains active[^2].
...
* The `kraft cloud` [command-line tool reference](/cli/), and in particular the [deploy](/cli/deploy) and [instance create](/cli/instance/create) subcommands.
