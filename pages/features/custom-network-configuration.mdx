...
Custom network configuration is only available as part of enterprise plans, and it requires a valid license.
...
In the meantime, configure networking through the [API](/api/platform/v1/instances), which you can invoke with `curl` or the [`unikraft api`](/cli/unikraft/api) command.
:::
{/* vale on */}
