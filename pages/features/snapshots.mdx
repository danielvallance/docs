...
When you enable stateful mode and first deploy your app, Unikraft Cloud brings it up
normally and lets it fully initialize (which may be fast, or take seconds or
minutes, depending on your app's init time). Unikraft Cloud waits until the app's exposed port
becomes available, and then starts the scale-to-zero process (assuming no traffic is coming
to that port). At this point, Unikraft Cloud saves the state of your app, and sets the
