...
Instance templates used to be node-local: every controller had to build its own copy before cloning instances from it.
...
`GET /v1/images` now also reports the state of each image:
