...
```bash title="unikraft"
unikraft image delete <my-org>/<my-app>[:<tag>]
```
...
When you push an update to an existing image, the CLI checks whether the kernel layer is already present in the registry and skips re-uploading it if so.
