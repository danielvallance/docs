...
<CodeTabs syncKey="registry">
  ```bash title="central registry"
  unikraft build . --output index.unikraft.io/<username>/<instance-name>:latest
  unikraft run --metro=fra -p 443:8080/http+tls -m 256MiB --name <instance-name> index.unikraft.io/<username>/<instance-name>:latest
  ```
  ```bash title="local registry"
  unikraft build . --output index.<node-name>.unikraft.cloud/<username>/<instance-name>:latest
  unikraft run --metro=fra -p 443:8080/http+tls -m 256MiB --name <instance-name> index.<node-name>.unikraft.cloud/<username>/<instance-name>:latest
  ```
</CodeTabs>
