...
```bash title="unikraft"
unikraft build . --output my-org/http-erlang:latest
unikraft run --metro=fra -p 443:8080/http+tls -m 512MiB my-org/http-erlang:latest
```
...
```bash title="unikraft"
unikraft instances delete erlang-sw2bp
```
