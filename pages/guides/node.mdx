...
```bash title="unikraft"
unikraft build . --output my-org/http-node21:latest
unikraft run --metro=fra -p 443:8080/http+tls -m 256MiB my-org/http-node21:latest
```
...
```bash title="unikraft"
unikraft instances delete http-node21-ubl8g
```
