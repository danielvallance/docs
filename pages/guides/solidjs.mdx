...
```bash title="unikraft"
unikraft build . --output my-org/node21-solid-start:latest
unikraft run --metro=fra -p 443:3000/http+tls -m 256MiB my-org/node21-solid-start:latest
```
...
```bash title="unikraft"
unikraft instances delete node21-solid-start-lvoa2
```
