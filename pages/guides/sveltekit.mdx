...
```bash title="unikraft"
unikraft build . --output my-org/node21-sveltekit:latest
unikraft run --metro=fra -p 443:3000/http+tls -m 256MiB my-org/node21-sveltekit:latest
```
...
```bash title="unikraft"
unikraft instances delete node21-sveltekit-zmt39
```
