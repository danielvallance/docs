...
Instead of constantly polling for updates, webhooks allow services to push data to your app the moment something happens—a commit is pushed, a payment is processed, or an issue is created.
...
Unikraft Cloud's unikernel architecture provides:
...
* Instant wake-up ensures events are processed immediately upon arrival.
...
Unfortunately, webhooks are not a standardized protocol, so each service has its own conventions.
We recommend to also follow [Webhooks.fyi](https://webhooks.fyi/) for best practices.
...
- [Express docunmentation](https://expressjs.com/)
